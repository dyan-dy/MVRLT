...
# 设置设备为 GPU（可选）
bpy.context.preferences.addons["cycles"].preferences.compute_device_type = "CUDA"  # 或 "OPTIX" / "HIP" / "ONEAPI"
...
def cam_load(views):
    import json
    num_views=64
    cam_pose_dir = 'assets/multview_pose/cam_pose_' + str(num_views) + '.json'
    if (Path(cam_pose_dir).exists()):
        with open(cam_pose_dir, 'r') as f:
            print("\nAlready exists views: "+ cam_pose_dir)
            ...
            for glb_path in os.listdir(sub_dir_path):
                ...
                if glb_path.endswith('.glb'):
                    ...
                    if arg.object.endswith(".blend"):
                        ...
                        init_scene()
                        print(f" arg.output_folder: { arg.output_folder}   arg.object: {arg.object}")
                        load_object(arg.object)
                        if arg.split_normal:
                            split_mesh_normal()
                    scale, offset = normalize_scene()
                    cam = init_camera()
                    print(f'[INFO] Processing object: {arg.object} \n\n into {arg.output_folder} \n\n')
                    ...
                    for i, view in enumerate(views):
                        cam.location = (cam_poses[i]['location'][0],cam_poses[i]['location'][1],cam_poses[i]['location'][2])
                        cam.data.lens = cam_poses[i]['lens']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--object_all_dir', type=str, default=None,help='Path to all the 3D model file to be rendered.')
